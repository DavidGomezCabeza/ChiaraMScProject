function [ NegativeArea ] = NewNegativePenlty_Liquid2( Phase, SpecData, PeakInfo, IdentifyResult, Weight, L , a_num, FinalPhase )
...
%     Phase(2) = Inf;
% end
FinalData = SpecData .* exp( sqrt(-1) * ( FinalPhase( 1 )*0+ Phase( 2 )*a_num  ) );
...
for i = 1 : length( PeakInfo )
    ...
    if( IdentifyResult( i ) == -1 ) %negative
        ...
        % non metto il - perché se mettessi il meno implicherebbe che il
        % picco negativo riventa positivo e quindi questo non renderebbe
        % possibile avere una differenza tra spettro reale e il suo valore
        % assoluto 
    end
end
