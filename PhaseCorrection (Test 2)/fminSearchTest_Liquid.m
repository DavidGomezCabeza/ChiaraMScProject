function [x,fval,exitflag,output] = fminSearchTest_Liquid(funfcn,x,options,varargin)
...
% DA ELIMINARE ------
% funfcn = @( Phase )NewNegativePenlty_Liquid( Phase, PhaseDataAfterCon, PeakInfo, IdentifyResult, Weight, DataSize ); 
% x = [0 0];
% options = options;
...
zero_term_delta = 0.1;      % Even smaller delta for zero elements of x
for j = 1:n
    y = xin;
    if y(j) ~= 0
